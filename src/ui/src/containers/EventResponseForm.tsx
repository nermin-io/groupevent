...
const EventResponseForm: React.FC<Props> = ({ invite, answer }) => {
  ...
  if (responded)
    return (
      <ResponseMessage invite={invite} response={parseResponse(response)} />
    );
    ...
    <FormContainer>
      ...
      {isCancelled(invite) && (
        <Notification
          ...
          description="You are no longer able to respond to this invite."
        />
      )}
      {hasLapsed(invite) && (
        <Notification
      ...
      <Flex css={{ gap: 12, flexDirection: "column" }}>
        ...
        <Button
          disabled={!valid || isCancelled(invite) || hasLapsed(invite)}
          onClick={handleSendEventResponse}
